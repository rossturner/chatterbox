...
def load_audio_samples_v2(audio_dir: str) -> List[AudioSample]:
    ...
    for audio_path in tqdm(audio_files, desc="Processing audio"):
        try:
            ...
            if not transcript:
                print(f"Warning: Empty transcript for {audio_path.name}, skipping...")
                continue
            
            print(f"Loaded {audio_path.name}: '{transcript[:50]}...'")
    ...
    print(f"Successfully loaded {len(samples)} samples with transcriptions")
    return samples
...
def collate_fn(samples):
    """
    Smart collate function that pads to batch maximum, not global maximum.
    This prevents the model from learning that all audio is the same length.
    """
    # Find max lengths in THIS batch only
    ...
    for sample in samples:
        audio = sample['audio']
        audio_16k = sample['audio_16k']
        
        # Pad to batch max only if needed
        if len(audio) < max_audio_len:
            pad_amount = max_audio_len - len(audio)
            audio = torch.cat([audio, torch.zeros(pad_amount)])
        
        if len(audio_16k) < max_audio_16k_len:
            pad_amount = max_audio_16k_len - len(audio_16k)
            audio_16k = torch.cat([audio_16k, torch.zeros(pad_amount)])
        
        audio_batch.append(audio)
        audio_16k_batch.append(audio_16k)
    
    return {
        'audio': torch.stack(audio_batch),
        'audio_16k': torch.stack(audio_16k_batch),
        'text': [s['text'] for s in samples],
        'audio_path': [s['audio_path'] for s in samples],
        'original_durations': [s['original_duration'] for s in samples],
    }
